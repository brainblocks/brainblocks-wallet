...
- For class components, the ordering of methods should be:
  ...
  - event handlers
  - render
- (Request for comment) For function components, destructure props in the parameters. Always include `...rest` and spread it to the root element of your component `<div className={styles.root} {...rest}> ... </div>`. It is good practice to destructure so that you can safely spread `rest` to the root element, and avoid writing `props.myParameter` throughout.
